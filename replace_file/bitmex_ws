...
class BitMEXWebsocket:
    ...
    def get_ticker(self):
        ...
        while True:
            try:
                ...
                # print(ticker)
                return (lastTrade['timestamp'], round(float(ticker["last"]), 1))
            except KeyError:
                self.logger.info('WARNING: GET_TICKER IS AWAITING WS.')
                sleep(0.5)
                continue
    ...
    def __on_error(self, ws, error):
        ...
        self.logger.info('FATAL WEBSOCKET ERROR. ATTEMPTING TO RECONNECT...')
        # self.exit()
        sleep(1.5)
